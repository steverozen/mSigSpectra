test_that("read_vcf (fread) reads a Mutect VCF and drops ## header lines", {
  out <- read_vcf(extdata("Mutect-GRCh37", "Mutect.GRCh37.s1.vcf"))
...
test_that("read_vcf default filter=TRUE keeps PASS / . / empty rows", {
  unfiltered <- read_vcf(extdata("Mutect-GRCh37", "Mutect.GRCh37.s1.vcf"),
                         filter = FALSE)
  filtered <- read_vcf(extdata("Mutect-GRCh37", "Mutect.GRCh37.s1.vcf"),
                       filter = TRUE)
...
test_that("read_vcf accepts a custom filter character vector", {
  passing <- read_vcf(extdata("Mutect-GRCh37", "Mutect.GRCh37.s1.vcf"),
                      filter = "PASS")
  expect_true(all(passing$FILTER == "PASS"))
})

test_that("read_vcf reads a Strelka SBS VCF (FILTER-agnostic)", {
  out <- read_vcf(extdata("Strelka-SBS-GRCh37", "Strelka.SBS.GRCh37.s1.vcf"))
  expect_gt(nrow(out), 0L)
  expect_true(all(nchar(out$REF) == 1L & nchar(out$ALT) == 1L))
})

test_that("read_vcf reads a Strelka ID VCF", {
  out <- read_vcf(extdata("Strelka-ID-GRCh37", "Strelka.ID.GRCh37.s1.vcf"))
  expect_gt(nrow(out), 0L)
