...
real Strelka VCF that ships with the package. It then plots each
...
`variant_type = "ID"`. The annotator left-justifies each indel,
extracts the local repeat / microhomology context, and emits the three
...
`vcfs_to_catalogs()` is a thin wrapper that reads, splits, annotates,
and column-binds catalogs from a list of files.
...
* `?annotate_vcf` — sequence context + transcript strand + (for ID)
  indel justification + classification.
